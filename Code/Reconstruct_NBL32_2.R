...
svaba_filtered = svaba_filtered[AD_TUMOR>(10*median_AD)] 
...
gg$edges$mark(lwd=1)#log10(gg$edges$dt$AD_TUMOR))
plot(gg$gt, window)
...
AD_threshold = floor(quantile(gg$edges$dt$AD_TUMOR, 0.75, na.rm=T))
...
for (i in 1:length(gg$nodes)){
  ...
  if (any(this_edges$AD_TUMOR > AD_threshold, na.rm=T)){  # if at this side of the node, there is any deep node
  ...
  this_edges = gg$edges$dt[this_edges]
  if (any(this_edges$AD_TUMOR > AD_threshold, na.rm=T)){
    #gg[,(type == "REF" & (edge.id %in% this_edges$edge.id))]
...
# gg.sub$edges$mark(lwd=log10(gg.sub$edges$dt$AD_TUMOR))
...
# gg.sub$edges$mark(deepEdge = gg.sub$edges$dt$AD_TUMOR>=deep_edge_threshold)
...
enhancer_of_interest = lowMYCN_aggregate_peaks %>%
  filter(seqnames == "chr2", start == 6662574)
...
window = GRanges(
  ...
  ranges = IRanges(start = c(15100000, 16800000, 17400000, 18000000),
                   end   = c(16200000, 17100000, 17800000, 18250000))
)
...
#gg_forplotting$edges$mark(lwd=0.0001*(gg_forplotting$edges$dt$AD_TUMOR))
...
pdf("/Volumes/Elements/MYCNAmplicon/Results/NBL32.pdf", height=7, width=7, onefile=F, useDingbats = F)
plot(c(gencode, gt.copynumber, sv.gt, reconstruction.gt, gt.NoMYCNExpr_H3K27ac, noMYCN_aggregate_peaks.gt, gt.LowMYCNExpr_H3K27ac, lowMYCN_aggregate_peaks.gt, gt.CLBGA_PHOX2B, gt.CLBGA_GATA3, gt.CLBGA_HAND2), window,
