...
# bcftools view somatic_run.svaba.unfiltered.somatic.sv.vcf.gz --regions 2:10000000-75000000 > NBL32.svaba.unfiltered.somatic.sv.vcf.chr2_10M_75M.vcf
...
svaba_filtered = svaba_filtered[AD_TUMOR>10 & SPAN > 1000] 
...
wgs_cov_cov = ascat_cnv_gr %>% filter(Sample == "NBL32") %>%
  coverage(weight = "TumorTotalCopyNumber")
...
#save.image("/Volumes/Elements/MYCNAmplicon/Results/NBL32Reconstruction_PLAYGROUND.Rdata")
#load("/Volumes/Elements/MYCNAmplicon/Results/NBL32Reconstruction_PLAYGROUND.Rdata")
...
window = GRanges(
  seqnames = rep("2", 1),
  ranges = IRanges(start = c(15000000),
                   end   = c(19000000))
)
...
gt.NoMYCNExpr_H3K27ac = gTrack("/Volumes/Elements/MYCNAmplicon/Results/Boeva_nonMYCNExpr_H3K27ac_MeanFC.bw", bar=TRUE, name = "Non MYCN Expr NB (H3K27ac FC)", y0=0, y1=10)
gt.LowMYCNExpr_H3K27ac = gTrack("/Volumes/Elements/MYCNAmplicon/Results/Boeva_lowMYCNExpr_H3K27ac_MeanFC.bw", bar=TRUE, name = "Low MYCN Expr NB (H3K27ac FC)", y0=0, y1=10)
gt.CLBGA_GATA3 = gTrack("/Volumes/Elements/nb-cl-chipseq-results/bam/Boeva_CLB-GA_GATA3.trimmed.bwa_hg19.rmdup.filtered.bw", bar=TRUE, name = "CLB-GA GATA3", y0=0, y1=5)
gt.CLBGA_PHOX2B = gTrack("/Volumes/Elements/nb-cl-chipseq-results/bam/Boeva_CLB-GA_PHOX2B.trimmed.bwa_hg19.rmdup.filtered.bw", bar=TRUE, name = "CLB-GA PHOX2B", y0=0, y1=5)
gt.CLBGA_HAND2 = gTrack("/Volumes/Elements/nb-cl-chipseq-results/bam/Boeva_CLB-GA_HAND2.trimmed.bwa_hg19.rmdup.filtered.bw", bar=TRUE, name = "CLB-GA HAND2", y0=0, y1=5)

pdf("/Volumes/Elements/MYCNAmplicon/Figures/NBL32_Overview.pdf", height=10, width=7, onefile=F, useDingbats = F)
plot(c(gencode, gg.sub$gt, gt.copynumber, gt.LowMYCNExpr_H3K27ac, gt.NoMYCNExpr_H3K27ac, gt.CLBGA_PHOX2B, gt.CLBGA_GATA3, gt.CLBGA_HAND2), window)
dev.off()
